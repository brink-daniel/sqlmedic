...
<Script>
	...
	<Description>Page Life Expectancy (PLE) is the average time, in seconds, that pages stay in the buffer pool before they are flushed out by queries needing different data. The more data kept in memory, the better query performance will be as it avoids the time cost of having to access the physical storage. The PLE figure changes constantly, monitor it frequently over a long period to create a performance baseline/trend.
	...
	Your PLE be improved by adding more memory, but the real fix is to enhance indexes and code quality - fix those large scans caused by missing indexes or code that is not SARGable.</Description>
	<Pass></Pass>	
	<Fail></Fail>
	<Check>false</Check>
